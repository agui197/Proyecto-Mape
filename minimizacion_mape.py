...
def genX(lmin=0,lmax=20,npoints=29):
    x1 = linspace(lmin,lmax,npoints)
    x2 = linspace(lmin,lmax,npoints)
...
def testfunction3(X,noise=False):
    ...
    den[den==0]=0.001
    if noise:
        Y = 10+sin(sqrt(X1**2+X2**2))/den+(0.5*random.rand(X.shape[0])-0.25)
    else:
        Y = 10+sin(sqrt(X1**2+X2**2))/den
...
for kernel in ['rbf', 'linrbf']:
    ...
    results.to_csv('results_train_'+kernel+'_D'+'.csv')
    hiperparams.to_csv('hiperparams_'+kernel+'_D'+'.csv')
